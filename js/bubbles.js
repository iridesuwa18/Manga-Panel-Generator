...
// Fading ("wavy tail") bubble. The tail is now built as ONE continuous
...
function _ellipseCircumference(rx, ry) {
...
function svgDashedBubble(svg,cx,cy,rx,ry,b,sw) {
  ...
  const circumference = _ellipseCircumference(rx, ry);
...
// Spiked bubble: two independent length controls.
...
function svgSpikedBubble(svg,cx,cy,rx,ry,b,sw) {
  ...
  const otherLen = b.spikeLen != null ? b.spikeLen : 0;
...
function syncBubbleEditorFields(b) {
  ...
  set('bp-spike-len', b.spikeLen != null ? b.spikeLen : 0);
