...
// ── Snap-to-panel-edges toggle (was declared/used but had no UI) ──
function setSnapEnabled(val) {
  snapEnabled = !!val;
  window.showToast?.(snapEnabled ? 'Snap to panel edges on' : 'Snap to panel edges off');
}
window.setSnapEnabled = setSnapEnabled;
...
// ── Per-type field visibility in the Bubble Editor ────────────
// Tail Angle/Length/Breadth only apply to types that actually draw a
// tail; Dot/Spike/Dash counts each only affect one or two shapes'
// outline math (see buildBubbleSVG's switch). Previously every field
// showed for every type, including square/rectangle (no tail at all).
const BUBBLE_HAS_TAIL = new Set(['circle', 'bold', 'thought', 'fading', 'dashed', 'spiked', 'lilypad']);
function updateBubbleFieldVisibility(type) {
  const hasTail = BUBBLE_HAS_TAIL.has(type);
  const setDisp = (id, show) => { const el = document.getElementById(id); if (el) el.style.display = show ? '' : 'none'; };
  setDisp('bp-tail-title', hasTail);
...
function applyBubbleClip(wrap, b) {
  ...
  // Was reading a #strokeWidth DOM element that no longer exists (ui.js
  // never created it) — silently always fell back to 8. Panel border
  // width now lives in state.js's panelStrokeWidth, set via the Panel
  // Style controls in the Panel Editor tab.
  const sw = (panelStrokeWidth || 8) / 2;
...
function syncBubbleEditorFields(b) {
  ...
  updateBubbleFieldVisibility(b.type);

  // Clip-to-panel select: populate from this page's panel rects (same
  // source refreshQuickBubblePanelSel uses) and reflect the bubble's
  // current clipPanel. Was previously not wired to any input at all.
  const clipSel = document.getElementById('bp-clip-panel');
  if (clipSel) {
    const pg = selectedBubble?.pgKey;
    const all = (_lastPanelRects[pg]?.length ? _lastPanelRects[pg] : _lastBaseRects[pg]) || [];
    const rects = all.filter(r => r.pnl);
    clipSel.innerHTML = '<option value="">None</option>' +
      rects.map((r, i) => `<option value="${i}">Panel ${i + 1}</option>`).join('');
    clipSel.value = (b.clipPanel === null || b.clipPanel === undefined) ? '' : b.clipPanel;
  }
}
...
function bpUpdate(field, val) {
  ...
  if (field === 'type') updateBubbleFieldVisibility(val);
